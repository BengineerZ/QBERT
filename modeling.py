...
class BertEmbeddings_Quant(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    def __init__(self, config):
        super(BertEmbeddings_Quant, self).__init__()
        self.word_embeddings = QuantEmbedding(config.vocab_size, config.hidden_size, padding_idx=0)
        self.position_embeddings = QuantEmbedding(config.max_position_embeddings, config.hidden_size)
        self.token_type_embeddings = QuantEmbedding(config.type_vocab_size, config.hidden_size)
...
class BertSelfAttention_Quant(nn.Module):
    def __init__(self, config):
        super(BertSelfAttention_Quant, self).__init__()
...
class BertSelfOutput_Quant(nn.Module):
    def __init__(self, config):
        super(BertSelfOutput_Quant, self).__init__()
...
class BertAttention_Quant(nn.Module):
    def __init__(self, config):
        super(BertAttention_Quant, self).__init__()
        self.self = BertSelfAttention_Quant(config)
        self.output = BertSelfOutput_Quant(config)
...
class BertIntermediate_Quant(nn.Module):
    def __init__(self, config):
        super(BertIntermediate_Quant, self).__init__()
        self.dense = QuantLinear(config.hidden_size, config.intermediate_size)
...
class BertOutput_Quant(nn.Module):
    def __init__(self, config):
...
class BertLayer_Quant(nn.Module):
    def __init__(self, config):
        super(BertLayer_Quant, self).__init__()
        self.attention = BertAttention_Quant(config)
        self.intermediate = BertIntermediate_Quant(config)
        self.output = BertOutput_Quant(config)
...
class BertEncoder_Quant(nn.Module):
    def __init__(self, config):
        super(BertEncoder_Quant, self).__init__()
        layer = BertLayer_Quant(config)
...
class BertPreTrainedModel_Quant(nn.Module):
    """ An abstract class to handle weights initialization and
        a simple interface for dowloading and loading pretrained models.
    """

    def __init__(self, config, *inputs, **kwargs):
        super(BertPreTrainedModel_Quant, self).__init__()
        if not isinstance(config, BertConfig_Quant):
            raise ValueError(
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        config_object = kwargs.get('config', None)
        ...
        config = BertConfig_Quant.from_json_file(config_file)
        # Instantiate model.
        if config_object is not None:
            assert isinstance(config_object, BertConfig_Quant)
            config_object.vocab_size = config.vocab_size
            config = config_object
            logger.info("[USING CONFIG OBJECT] Model config {}".format(config))
...
class BertConfig_Quant(object):
    ...
    def from_dict(cls, json_object):
        ...
        config = BertConfig_Quant(vocab_size_or_config_json_file=-1)
...
class BertModel_Quant(BertPreTrainedModel_Quant):
...
class BertForSequenceClassification_Quant(BertPreTrainedModel_Quant):
    ...
    def __init__(self, config, num_labels):
        super(BertForSequenceClassification_Quant, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel_Quant(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertForQuestionAnswering_Quant(BertPreTrainedModel_Quant):
    ...
    def __init__(self, config):
        super(BertForQuestionAnswering_Quant, self).__init__(config)
        self.bert = BertModel_Quant(config)
...
class BertForTokenClassification_Quant(BertPreTrainedModel_Quant):
    ...
    def __init__(self, config, num_labels=2):
        super(BertForTokenClassification_Quant, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel_Quant(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
